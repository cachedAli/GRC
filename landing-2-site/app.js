...
(function () {
  const LIVE_APP_BASE = 'http://localhost:3000';
  const LOCAL_LIVE_HOSTS = ['localhost', '127.0.0.1'];
  const canUseLocalLiveApp = LOCAL_LIVE_HOSTS.indexOf(window.location.hostname) !== -1;
  ...
  function renderFrameworkShowcase() {
    ...
    if (grid) {
      grid.innerHTML = list
        .map(
          (f) => `<article class="framework-grid-card reveal" style="--accent:${f.accent || '#0057ff'}">${frameworkBadgeHTML(f, 'framework-grid-mark')}<h3>${f.name}</h3><p>${f.subtitle || 'Framework coverage'}</p><div class="framework-grid-tags"><span>${f.region || 'Global'}</span><span>${f.category || 'Security'}</span></div></article>`
        )
        .join('');
    }
  }
  ...
  function paintExperience() {
    ...
    if (expLive && canUseLocalLiveApp && frame) {
      frame.src = LIVE_APP_BASE + expCurrent.path;
      ...
      if (status) {
        status.textContent = !canUseLocalLiveApp
          ? 'Illustrated · hosted standalone landing site'
          : isDeepPage()
  ...
  function renderExperience() {
    ...
    if (toggle) {
      if (!canUseLocalLiveApp) {
        ...
        toggle.title = 'Live product mode is disabled on the deployed standalone landing site';
      }
      toggle.addEventListener('click', () => {
        if (!canUseLocalLiveApp) return;
        expLive = !expLive;
